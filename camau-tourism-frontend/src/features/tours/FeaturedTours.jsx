...
export default function TourList({ limit, showSearch = true }) {
  ...
  useEffect(() => {
    async function fetchTours() {
      ...
      setError("");
      try {
        const data = await getList("tours");
        const dataShowActive = data.filter(k => k.is_active);
        setTours(dataShowActive);
      } catch (err) {
  ...
  return (
    <div className="bg-gradient-to-br from-cyan-50 via-blue-50 to-indigo-50 min-h-screen pb-20">
      {/* Hiệu ứng decorative background */}
      ...
      <div className="max-w-7xl mx-auto px-4 pt-8 relative z-10">
        ...
        {loading && (
          <div className="flex flex-col items-center py-28 text-cyan-600 font-bold text-xl">
            ...
            <p className="animate-pulse">Đang khám phá các tour tuyệt vời...</p>
          </div>
        )}
